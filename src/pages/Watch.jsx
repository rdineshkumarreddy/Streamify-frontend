                ...
                {comments.length > 0 ? (
                  comments.map(comment => (
                    <div key={comment._id} className="flex space-x-2 sm:space-x-3">
                      <div className="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gray-700 flex items-center justify-center">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 mt-1.5 sm:mt-2 ml-1 sm:ml-2 text-[10px] sm:text-xs text-gray-400">
                          <button 
                            onClick={() => handleCommentLike(comment._id)}
                            className={`flex items-center space-x-1 ${comment.isLiked ? 'text-blue-500' : 'hover:text-white'}`}
                          >
                            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill={comment.isLiked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {activeReplyCommentId === comment._id && (
                          <form onSubmit={(e) => handleReplySubmit(e, comment._id)} className="mt-2.5 ml-2 sm:ml-6 flex space-x-2 sm:space-x-3">
                            <div className="flex-shrink-0 h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-gray-700 flex items-center justify-center">
                              {user?.avatar ? (
                                <img 
                                  src={user.avatar} 
                                  alt={user.username} 
                            ...
                            <div className="flex-1">
                              <input
                                ...
                                className="w-full bg-transparent border-b border-gray-700 focus:border-blue-500 focus:outline-none text-white text-xs py-1"
                                autoFocus
                              />
                              <div className="flex justify-end mt-1">
                                <button
                                  ...
                                  className="px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-[11px] text-gray-400 hover:text-white mr-1"
                                >
                                  Cancel
                                </button>
                                <button
                                  ...
                                  className={`px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-[11px] ${replyText.trim() ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-700 text-gray-500 cursor-not-allowed'}`}
                                >
                                  Reply
                                </button>
                              </div>
                        ...
                        {comment.replies && comment.replies.length > 0 && (
                          <div className="mt-3.5 space-y-3.5 ml-2.5 sm:ml-6 border-l border-gray-800 pl-3 sm:pl-4">
                            {comment.replies.map(reply => (
                              <div key={reply._id} className="flex space-x-2 sm:space-x-3">
                                ...
                                <div className="flex-1">
                                  <div className="bg-gray-850 rounded-lg p-2 sm:p-2.5">
                                    ...
                                    <p className="text-gray-300 text-[11px] sm:text-xs">{reply.content}</p>
                                  </div>
                                  <div className="flex items-center space-x-4 mt-1 ml-1 text-[9px] sm:text-[10px] text-gray-500">
                                    <button 
                                      onClick={() => handleCommentLike(reply._id)}
                                      className={`flex items-center space-x-1 ${reply.isLiked ? 'text-blue-500' : 'hover:text-white'}`}
                                    >
                                      <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill={reply.isLiked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
                                      </svg>
